...
import com.apitechnosoft.ipad.filepicker.animation.SimpleValueAnimator;
import com.apitechnosoft.ipad.filepicker.animation.SimpleValueAnimatorListener;
import com.apitechnosoft.ipad.filepicker.animation.ValueAnimatorV14;
import com.apitechnosoft.ipad.filepicker.animation.ValueAnimatorV8;
 ...
 * @author AST Inc.
 */
public class ASTCropImageView extends AppCompatImageView {
	...
	public ASTCropImageView(Context context) {
		this(context, null);
	}

	public ASTCropImageView(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public ASTCropImageView(Context context, AttributeSet attrs, int defStyle) {
	...
	private Bitmap scaleBitmapIfNeeded(Bitmap cropped) {
		...
		if (outWidth > 0 && outHeight > 0) {
			Bitmap scaled = ASTImageUtil.getScaledBitmap(cropped, outWidth, outHeight);
	...
	public void startCrop(final MediaFile file, CropCallback cropCallBack) {
		...
		mExecutor.submit(new Runnable() {
			...
			public void run() {
				...
				file.setPath(ASTImageUtil.saveToFile(cropped, mCompressFormat, mCompressQuality));
